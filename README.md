...
- Renaming a tag doesn't check for name collisions with another existing tag —
  low risk in practice, but two tags could technically end up with the same name.
...
- Tag rename + color picker (color dot shown in tag list)
- Note editing on existing tagged entries (previously add/delete only)
- Image export theme picker — 4 card styles (Navy & Gold, Parchment, Charcoal & Rose, Forest)
